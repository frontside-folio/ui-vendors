...
class AddNewVendor extends React.Component {
  ...
  render() {
    ...
    return (
      ...
      <form id="form-add-new-vendor" onSubmit={handleSubmit(this.saveSet)}>
        <Pane paneTitle="Add New Vendor" defaultWidth="60%" dismissible="true" lastMenu={lastMenu} onClose={this.props.onCloseDetails}>
  ...
  saveSet(data) {
    ...
    // mutable: true,
    console.log(data);
    this.props.parentMutator.vendor.POST(data).then(() => {
      console.log(data);
    });
  }
}
