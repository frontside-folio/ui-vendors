...
class ViewBarcodeScanner extends React.Component {
  ...
  render() {
    const lastMenu = (<PaneMenu>
      <button id="delete-scanner" onClick={() => { this.onDeleteAll() }} title="Scan" style={{ textAlign: 'center', display: 'flex' }}><Icon icon="trashBin" />Remove all</button>
    </PaneMenu>);
    const itemDataStatus = _.isEmpty(this.state.itemData) === false ? true : false; 
    return (
      <Pane paneTitle="Link Scanner" defaultWidth="50%" dismissible={true} onClose={this.props.onCloseDetails} lastMenu={lastMenu}>
        <div style={{width: '100%'}}>
  ...
  onDeleteAll() {
    var ref = firebase.database().ref('sessions/'); //root reference to your data
