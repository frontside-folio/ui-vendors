...
class EditVendor extends React.Component {
  ...
  render() {
    ...
    const lastMenu = (<PaneMenu>
      <Button type="button" buttonStyle="error paneHeaderNewButton" onClick={() => { this.onOpenModal() }} >Remove</Button>
  ...
  deleteVendor(ID) {
    this.props.parentMutator.localRes.replace({ id: null });
    this.props.parentMutator.vendor.DELETE({ id: ID }).then(()=>{
