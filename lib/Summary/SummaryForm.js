...
class SummaryForm extends React.Component {
  ...
  render() {
    ...
    return (
        <Row>
          ...
          <Col xs={12} md={6}>
            <Field label="Vendor Description" name="description" id='description' component={TextArea} fullWidth />
        </Col>
      </Row>
    );
  }
  ...
  renderSubFields = (elem, index, fields) => {
    return (
      <Row key={index}>
        <Col xs={5}>
          <Field label="name" name={`${elem}.value`} id={`${elem}.value`} component={TextField} fullWidth />
        </Col>
        <Col xs={5}>
