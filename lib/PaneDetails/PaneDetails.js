...
class PaneDetails extends React.Component {
  ...
  constructor(props) {
    ...
    this.getContactCategory = this.getContactCategory.bind(this);
    // Preloader
    this.panePreloader = this.panePreloader.bind(this);
  }

  componentWillReceiveProps(nextProps) {
    if (nextProps.resources.vendorGETID !== null) {
      const res = nextProps.resources;
      if (res.vendorGETID.hasLoaded !== false) {
        if (res.vendorGETID.records.length >= 0) {
          const newPreloader = res.localRes.panePreloader;
          if (newPreloader !== false) {
           this.props.mutator.localRes.update({ panePreloader: false });
         }
        }
      }
    }
  ...
  render() {
    const is_data = (this.getCategory() !== null && this.getContactCategory() !== null) ? true : false ;
      ...
      <div style={{width:'100%'}}>
        ...
        <Route
          ...
          render={props => <this.connectedViewVendor
            ...
            parentMutator={this.props.mutator}
            onCloseDetails={this.props.onCloseDetails}
            panePreloader={this.panePreloader()}
            {...props}
          />}
        />
      </div>
