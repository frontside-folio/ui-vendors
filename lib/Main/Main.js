...
class Main extends Component {
  static manifest = Object.freeze({
    ...
    vendorQuery: {
      ...
      records: 'vendors',
      path: 'vendor',
      recordsRequired: '%{localRes.resultCount}',
      perRequest: 30,
      params: {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.resources.vendorQuery !== null) {
      const res = nextProps.resources;
      if (res.vendorQuery.hasLoaded !== false) {
        if (res.vendorQuery.records.length >= 0) {
          const statePreloader = this.state.listPreloaderStatus;
          const newPreloader = res.localRes.listPreloaderStatus;
          if (statePreloader === newPreloader) {
            this.setState({ listPreloaderStatus: false });
          }
        }
      }
    }
  ...
  render() {
    ...
    const listPreloaderStatus = this.state.listPreloaderStatus;
    ...
    const loader = () => {``
      return this.props.resources.vendorQuery ? this.props.resources.vendorQuery.isPending : false;
    }
  ...
  setDefaultQuery() {
    ...
    this.props.mutator.localRes.update({
      query: `${query}`,
      filter: filter,
      sortBy: sortBy,
      sortOrder: sortOrder,
  ...
  onClearSearch = (e) => {
    ...
    this.props.mutator.localRes.update({ query: 'query=(name=*)', listPreloaderStatus: true });
  }
  onChangeSearch = (e) => {
    const query = e.target.value;
    const updateQuery = query.trim().length > 0 ? query : '*'
    this.props.mutator.localRes.update({ query: 'query=(name=' + updateQuery + '*)', listPreloaderStatus: true });
    this.setState({ searchTerm: query, listPreloaderStatus: true });
  }
  // Filters
  onChangeFilter = (e) => {
    const filters = Object.assign({}, this.state.filters);
    filters[e.target.name] = e.target.checked;
    this.setState({ filters, listPreloaderStatus: true });
  ...
  onSelectRow = (e, row) => {
    ...
    this.props.mutator.localRes.update({ id: `${row.id}`, panePreloader: true });
  ...
  onHeaderClick = (e, data) => {
    ...
    this.setState({ listPreloaderStatus: true });
