...
class Main extends Component {
  static manifest = Object.freeze({
    ...
    vendorQuery: {
      ...
      params: {
        limit: '1000',
        query: '%{localRes.query} %{localRes.filter} %{localRes.sort}'
      }
    },
    vendorGETID: {
      type: 'okapi',
  ...
  constructor(props) {
    super(props);
    const query = ParseURLFilter(props);
    this.state = {
  ...
  defaultQuery() {
    const filtersURL = ParseURLFilter(this.props);
    var query = 'query=(name=*)';
    var filter = filters2cql(filterConfig, filtersURL) ? 'and ' + filters2cql(filterConfig, filtersURL) : '';
    var sort= '';
    this.props.mutator.localRes.update({ query: `${query}`, filter: filter, sort: sort });
  }
